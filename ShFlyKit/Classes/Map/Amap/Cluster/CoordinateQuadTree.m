...
BoundingBox quadTreeNodeDataArrayForPOIs(QuadTreeNodeData *dataArray, NSArray * pois){
    CLLocationDegrees minX = ((AMapClusterModel *)pois[0]).location.latitude;
    CLLocationDegrees maxX = ((AMapClusterModel *)pois[0]).location.latitude;
    
    CLLocationDegrees minY = ((AMapClusterModel *)pois[0]).location.longitude;
    CLLocationDegrees maxY = ((AMapClusterModel *)pois[0]).location.longitude;
            ...
            QuadTreeGatherDataInRange(self.root, BoundingBoxForMapRect(mapRect), ^(QuadTreeNodeData data)
                                      {
                                          ...
                                          totalY += data.y;
                                          count++;
                                          [pois addObject:(__bridge AMapClusterModel *)data.data];
                                      });
            /* 若区域内仅有一个数据. */
            if (count == 1){
...
- (NSArray<ClusterAnnotation *> *)clusteredAnnotationsWithinMapRect:(MAMapRect)rect
                                                       withDistance:(double)distance {
    __block NSMutableArray<AMapClusterModel *> *allAnnotations = [[NSMutableArray alloc] init];
    QuadTreeGatherDataInRange(self.root, BoundingBoxForMapRect(rect), ^(QuadTreeNodeData data) {
        [allAnnotations addObject:(__bridge AMapClusterModel *)data.data];
    });
    
    NSMutableArray<ClusterAnnotation *> *clusteredAnnotations = [[NSMutableArray alloc] init];
    for (AMapClusterModel *aAnnotation in allAnnotations) {
...
- (ClusterAnnotation *)getClusterForAnnotation:(AMapClusterModel *)annotation
                        inClusteredAnnotations:(NSArray<ClusterAnnotation *> *)clusteredAnnotations withDistance:(double)distance {
