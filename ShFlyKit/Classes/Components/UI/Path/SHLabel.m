...
@interface SHLabel ()
@property(nonatomic,assign)BOOL gesturePathEnable;                                      //手势使用可用
@property(nonatomic,strong)SHLabelPath *path;                                           //路径
@property(nonatomic,strong)NSMutableArray<NSAttributedString *> *stringArray;
@property(nonatomic,strong)NSMutableArray<CATextLayer *> *layerMutableArray;
@property(nonatomic,strong)NSMutableArray *gesturePointArray;                           //手势路径点坐标数组
@property(nonatomic) SHLabelHorizontalAlignment textHorizontalAlignment;                //水平方向对齐，默认值为SHLabelHorizontalAlignmentLeft
@property(nonatomic) SHLabelVerticalAlignment textVerticalAlignment;                    //垂直方向对齐，默认值为SHLabelVerticalAlignmentCenter
@end
...
-(void)configSelf {
    ...
    _textHorizontalAlignment = SHLabelHorizontalAlignmentLeft;
    _textVerticalAlignment = SHLabelVerticalAlignmentCenter;
...
-(CGFloat)lineBeginX:(NSAttributedString *)string {
    switch (_textHorizontalAlignment) {
        case SHLabelHorizontalAlignmentLeft:
            return 0.0;
        case SHLabelHorizontalAlignmentCenter:
            return (self.bounds.size.width - string.size.width) / 2;
        case SHLabelHorizontalAlignmentRight:
...
-(CGFloat)beginY {
    switch (_textVerticalAlignment) {
        case SHLabelVerticalAlignmentUp:
            return 0.0;
        case SHLabelVerticalAlignmentCenter:
            return (self.bounds.size.height - _attributedText.size.height) / 2;
        case SHLabelVerticalAlignmentDown:
...
#pragma mark - 文本路径
@implementation SHLabel (Path)
-(SHLabelPath *)path {
    return _path;
}


-(void)setPath:(SHLabelPath *)path {
...
@implementation SHLabel (gesturePath)
...
-(void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if (_gesturePathEnable && _gesturePointArray != nil && _gesturePointArray.count > 0) {
        SHLabelPath *path = [SHLabelPath pathForBeginPoint:((NSValue *)_gesturePointArray[0]).CGPointValue];
...
@implementation SHLabel (Animation)
...
@implementation SHLabel (ShowAndHide)
#pragma mark -直线移动动画
-(void)startShowWithDirection:(SHLabelAnimationDirection)direction duration:(NSTimeInterval)duration bounce:(CGFloat)bounce stepTime:(NSTimeInterval)stepTime {
    ...
    for (int i = 0; i < self.layerArray.count; i++) {
        ...
        switch (direction) {
            case SHLabelAnimationDirectionDown: {
            ...
            case SHLabelAnimationDirectionRight: {
                ...
                break;
            case SHLabelAnimationDirectionLeft: {
        ...
        if (direction == SHLabelAnimationDirectionRight) {
...
-(void)startHideWithDirection:(SHLabelAnimationDirection)direction duration:(NSTimeInterval)duration stepTime:(NSTimeInterval)stepTime {
    for (int i = 0; i < self.layerArray.count; i++) {
        ...
        if ((direction != SHLabelAnimationDirectionRight && i == self.layerArray.count - 1)
            || (direction == SHLabelAnimationDirectionRight && i == 0)) {
...
-(void)startFixedHideWithTransform:(CATransform3D *)transform duration:(NSTimeInterval)duration stepTime:(NSTimeInterval)stepTime {
    for (int i = 0; i < self.layerArray.count; i++) {
        ...
        opacityAnimation.keyTimes = @[@(0.0),@(fixedTime / (duration + fixedTime)),@(1.0)];
        opacityAnimation.duration = duration + fixedTime;
